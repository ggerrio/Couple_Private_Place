...
export default function HomeView() {
  ...
  const [spotifyToken, setSpotifyToken] = useState(() => localStorage.getItem("spotify_sdk_token") || "");
  const [isSdkMode, setIsSdkMode] = useState(() => !!localStorage.getItem("spotify_sdk_token"));
  ...
  useEffect(() => {
    ...
    localStorage.setItem("spotify_sdk_token", spotifyToken);
    ...
    const initPlayer = () => {
      ...
      player.addListener("ready", ({ device_id }: { device_id: string }) => {
        ...
        awardXp(10, "connecting your real Spotify Player SDK!");
      });
      ...
      player.addListener("initialization_error", ({ message }: { message: string }) => {
        setSpotifyErrorState(`Initialization Error: ${message}`);
        setIsSdkConnected(false);
      });

      player.addListener("authentication_error", ({ message }: { message: string }) => {
        setSpotifyErrorState("Authentication Error: Token might be expired. Get a new one!");
        setIsSdkConnected(false);
      });

      player.addListener("account_error", ({ message }: { message: string }) => {
        setSpotifyErrorState(`Account Error: Premium account is required. ${message}`);
        setIsSdkConnected(false);
      });
      ...
      player.addListener("player_state_changed", (state: any) => {
        ...
        if (currentTrack) {
          const updatedSong = {
            ...
            artwork: currentTrack.album.images[0]?.url || "https://images.unsplash.com/photo-1614680376593-902f74fa0d41?q=80&w=150&auto=format&fit=crop",
            durationMs: state.duration,
            progressMs: state.position,
            isPlaying: !state.paused,
            spotifyId: currentTrack.id,
          };
          syncSongToPartner(updatedSong);
        }
      });

      player.connect().then((success: boolean) => {
        if (success) {
          console.log("Connected successfully to Spotify Web Play!");
        }
      });
    ...
    return () => {
      // Keep player active to support continuous streaming, cleanup on complete unmount or token change
    };
  }, [spotifyToken, isSdkMode]);
  ...
  const handleProgressBarClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    syncSongToPartner({ ...currentSong, progressMs: newProg });
  };
  ...
  const handleSpotifyUrlSubmit = (e: React.FormEvent) => {
    ...
    // e.g., https://open.spotify.com/track/4PTG3Z6ehGkBF3zIqYQGS3?si=...
    // or spotify:track:4PTG3Z6ehGkBF3zIqYQGS3
    const regex = /(?:open\.spotify\.com\/track\/|spotify:track:)([a-zA-Z0-9]+)/i;
    const match = spotifyUrlInput.match(regex);
    if (match && match[1]) {
      const trackId = match[1];
      // Fetch or simulate fetching track details
      const newSong = {
        ...
        progressMs: 0,
        isPlaying: true,
        spotifyId: trackId // we save the custom spotify ID
      };

      syncSongToPartner(newSong);
      setSpotifyUrlInput("");
    } else {
  ...
  const handleTrackSkip = (direction: "next" | "prev") => {
    triggerHaptic("medium");
    if (isSdkConnected && spotifyPlayer) {
      if (direction === "next") {
        spotifyPlayer.nextTrack().catch((err: any) => console.error(err));
      } else {
        spotifyPlayer.previousTrack().catch((err: any) => console.error(err));
      }
      return;
    }

    // Local simulation skip
    if (isRepeatActive && direction === "next") {
      syncSongToPartner({ ...currentSong, progressMs: 0, isPlaying: true });
      return;
    }

    const currentIndex = SUGGESTED_VIBES.findIndex(
      v => v.title.toLowerCase() === currentSong.title.toLowerCase()
    );
    let nextIndex = 0;
    if (isShuffleActive && direction === "next" && SUGGESTED_VIBES.length > 1) {
      let rand = currentIndex;
      while (rand === currentIndex) {
        rand = Math.floor(Math.random() * SUGGESTED_VIBES.length);
      }
      nextIndex = rand;
    } else {
      if (direction === "next") {
        nextIndex = currentIndex === -1 ? 0 : (currentIndex + 1) % SUGGESTED_VIBES.length;
      } else {
        nextIndex = currentIndex === -1 ? SUGGESTED_VIBES.length - 1 : (currentIndex - 1 + SUGGESTED_VIBES.length) % SUGGESTED_VIBES.length;
      }
    }

    const vibe = SUGGESTED_VIBES[nextIndex];
    syncSongToPartner({
      title: vibe.title,
      artist: vibe.artist,
      album: vibe.album,
                ...
                {stickyNotes.map((note) => {
                  ...
                  return (
                    <motion.div
                      ...
                      style={{ rotate: note.id === "note-1" ? -2 : note.id === "note-2" ? 1.5 : note.id === "note-3" ? -1 : 1 }}
                    >
