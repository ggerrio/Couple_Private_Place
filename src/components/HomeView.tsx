...
// Neutral placeholder video used only until a real track is resolved.
const YT_PLACEHOLDER_VIDEO_ID = "5lYlS8n8nZ9Lh6p3O6Y2y2";
...
export default function HomeView() {
  ...
  const resolveYouTubeId = async (
    ...
    if (isYouTubeId(rawId)) return rawId as string;
    const found = await searchYouTubeVideo(artist, title);
    return found || YT_PLACEHOLDER_VIDEO_ID;
  };
    ...
    const unsubPlayback = onSnapshot(doc(db, "rooms", "spotify_room"), (snapshot) => {
      ...
      if (data.commandTriggeredBy !== currentUser || !hasInitialSyncDone.current) {
        ...
        if (data.isPlaying) {
          ...
          const isCurrentPlaying = ytPlaybackStateRef.current === 1;
          ...
          player.loadVideoById(data.spotifyId, targetPositionSeconds);
          player.playVideo();
        } else if (ytPlaybackStateRef.current === 1) {
          player.pauseVideo();
        }
      }
    });
  ...
  useEffect(() => {
    ...
    loadYouTubeIframeApi().then(() => {
      ...
      const player = new ytWindow.YT.Player("youtube-audio-engine", {
        height: "100%",
        width: "100%",
        videoId: YT_PLACEHOLDER_VIDEO_ID,
        playerVars: {
          autoplay: 0,
          controls: 0,
          disablekb: 1,
        ...
        events: {
          ...
          onStateChange: (event: any) => {
            if (isCancelled) return;
            ytPlaybackStateRef.current = event.data;
      ...
      ytPlayerLiveRef.current = player;
    });
  ...
  const handleSpotifyUrlSubmit = (e: React.FormEvent) => {
    ...
    const pastedInput = spotifyUrlInput;
    setSpotifyUrlInput("");

    if (match && match[1]) {
      // Direct YouTube link — id is already known and valid, no search needed.
      playYouTubeTrack(
        {
      ...
      // Spotify link — no title/artist to search with, so we search using the
      // track id as a best-effort query.
      searchYouTubeVideo("Spotify Pasted Track", spotifyMatch[1]).then((videoId) => {
        if (!videoId) {
          setSpotifyError("Could not find a matching YouTube video for that Spotify link.");
          return;
        }
        playYouTubeTrack(
          {
      ...
      // Treat as a raw search query
      searchYouTubeVideo("", pastedInput).then((videoId) => {
        if (!videoId) {
